...
#include "RooMinuit.h"
#include "RooNLLVar.h"
#include "/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/RooCBCrujffPdf.h"
#include "/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/RooCBCrujffPdf.cxx"
#include "/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/dict.h"
#include "/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/dict.cc"
...
string funcNames[nFuncs] = {"1pol","2pol","3pol","4pol","5pol","1exp","2exp","3exp","1pow","2powlow","3powlow","2lau","4lau","6lau","RooKey2","RooKey3","RooKey4","DiJet","DiJetPL","DiJetEXP","DiJetEXPOL",  "Expol","Exp","2Exp","ExpolPL","Lau","Bern", "ExpPAR"};

void checkFunc(string name){
  if (name!="1pol" && name!="2pol" && name!="3pol" && name!="4pol" && name !="5pol" && name!="1exp" && name!="2exp" && name!="3exp" && name!="1pow" && name!="2powlow" && name!="3powlow" && name!="2lau" && name!="4lau" && name!="6lau" && name!="BkgMCKeyPdf"&& name!="DiJet"&& name!="DiJetPL"&& name!="DiJetEXP"&& name!="DiJetEXPOL"&& name!="Expol"&& name!="ExpolPL"&& name!="Exp"&& name!="2Exp"&& name!="ExpPAR"&& name!="Lau"&& name!="Bern" && name!= "RooKey2"&& name!= "RooKey3"&& name!= "RooKey4") {
...
//   }
  ...
  else if (name=="ExpolPL"|| name=="Bern") return 4;
...
string getType(string name){
  ...
  else if (name=="Bern") return "Bern";
  else if (name=="2Exp") return "2Exp";
...
int main(int argc, char* argv[]){
 ...
 system("if [ ! -e \"/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/TestSWResults\" ]; then mkdir /afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/TestSWResults; fi");
 
 system("mkdir -p /afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/SWplots/toys");
 system("mkdir -p /afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/SWplots/data");
 system("mkdir -p /afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/SWplots/histos");
...
//par DiJetEXP
 ...
 //par Bern
 RooRealVar p0Bern("p0Bern", "p0Bern", 2.5, 0., 10.);
 RooRealVar p1Bern("p1Bern", "p1Bern", 0.9, 0., 5.);
 RooRealVar p2Bern("p2Bern", "p2Bern", 0.6, 0., 5.);
 RooRealVar p3Bern("p3Bern", "p3Bern", 0.4, 0., 5.);
 RooRealVar p4Bern("p4Bern", "p4Bern", 0.3, 0., 5.);
 ...
 if(!sigGen)  outFile = new TFile(Form("TestSWResults/biasCheck_g%s_f%s_m%d_w%d_cat%d_w%d_j%d_%s_FIXW_2406.root",genName.c_str(),fitName.c_str(),mMC,width,cat,winSize,jobNumb, suffix.c_str()),"RECREATE");
 else outFile = new TFile(Form("TestSWResults/biasCheck-sigGen_%g-g%s_f%s_m%d_w%d_cat%d_w%d_j%d_%s_FIXW.root",nSigGen, genName.c_str(),fitName.c_str(),mMC,width,cat,winSize,jobNumb,suffix.c_str()),"RECREATE");
 ...
 leg->AddEntry(h4,"sig part of s+b","l");
 ...
 if(genName =="Bern") genFcn = (RooGenericPdf*)genWS->pdf(Form("PhotonsMassBkg_Bern_truth_cat%d",cat));
 if(genName =="2Exp") genFcn = (RooGenericPdf*)genWS->pdf(Form("PhotonsMassBkg_2Exp_truth_cat%d",cat));
 ...
 genFcn3 = (RooGenericPdf*)genWS->pdf(Form("PhotonsMassBkg_Lau_truth_cat%d",cat));
  //se dovesse servire di generare i par iniziale con una multivariata
	...
	if(sigMCPdf==NULL){
      ...
      RooRealVar sigYield(Form("sigYield_m%d_cat%d",mMC,cat),Form("sigYield_m%d_cat%d",mMC,cat),nSigExp,-5000., 5000 );//+-100
 ...
 RooRealVar bkgGenEvents("bkgGenEvents","bkgGenEvents", 0,1e10);
  ...
  for (int itToy=jobNumb*toysPerJob; itToy<(jobNumb+1)*toysPerJob; itToy++){
    ...
    //	std::cout<<"Genero un num di eventi di bkg pari a qnt sn i dati"<<std::endl;
     ...
     paramsBkg->readFromFile("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/initParBkg.txt");
     ...
     if ( itToy>=0){
       ...
       RooPlot *mFrame = mass->frame(Title(Form("Gen: f_{1}. Fit: f_{0}. Mass %d  Width: %d cat %d  toy: %d ",mMC,width, cat,itToy )));
	 ...
	 genFcn->plotOn(mFrame,LineColor(kMagenta));
       }
