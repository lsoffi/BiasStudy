...
winSize=10
  ...
  for fitFun in  ExpPAR #DiJet  
    do
    for genFun in ExpPAR #DiJet Expol #DiJet ExpPAR # Expol DiJet #
      do
	...
	for cat in 0 #1 2 3
	 ...
	 for perc in 10 #0  
	  do
          for suffix in FIX
            do
	   echo   "hadd -f TestSWResults/mergeDir/biasCheck-sigGen_0-g${genFun}_f${fitFun}_m${mass}_w${perc}_cat${cat}_w${winSize}_${suffix}_new.root TestSWResults/biasCheck-sigGen_0-g${genFun}_f${fitFun}_m${mass}_w${perc}_cat${cat}_w${winSize}_j*_${suffix}.root"
	      
	      hadd -f TestSWResults/mergeDir/biasCheck-sigGen_0-g${genFun}_f${fitFun}_m${mass}_w${perc}_cat${cat}_w${winSize}_${suffix}_FIXW.root TestSWResults/biasCheck-sigGen_0-g${genFun}_f${fitFun}_m${mass}_w${perc}_cat${cat}_w${winSize}_j*_${suffix}.root
