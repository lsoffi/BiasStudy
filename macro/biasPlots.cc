...
void  MakeSinglePlotBias( Int_t mass ,  Int_t width,Int_t cat, Int_t winSize, std::string genFcn, std::string fitFcn, double* par) {
  ...
  TFile* fin= new TFile(Form("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/TestSWResults/mergeDir/biasCheck-sigGen_%d-g%s_f%s_m%d_w%d_cat%d_w10_%s.root", ngen,genFcn.c_str(), fitFcn.c_str(),mass,width, cat, range.c_str() ));
  std::cout<<Form("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/TestSWResults/mergeDir/biasCheck-sigGen_%d-g%s_f%s_m%d_w%d_cat%d_w10_%s.root", ngen,genFcn.c_str(), fitFcn.c_str(),mass,width, cat, range.c_str() )<<std::endl;
  ...
  //  ctmp->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"ErrRel_"+TString::Format("g%s_f%s_m%d_w%d_cat%d_w%d.png", genFcn.c_str(), fitFcn.c_str(), mass,width, cat, winSize));
  ...
  // ctmp->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"Err_"+TString::Format("g%s_f%s_m%d_w%d_cat%d_w%d.png", genFcn.c_str(), fitFcn.c_str(), mass,width, cat, winSize));
  ...
  h_b->Sumw2();
  tree->Draw("(sigYieldTot-sigYieldTotTruth)/sigYieldTotErr>>h_b", "sigYieldTotErr>0.00001 && abs((sigYieldTot-sigYieldTotTruth)/sigYieldTotErr)<5.5&&chi2>0.001&&fitStatus==0 && migradStatus==0&& abs(sigYieldTot)<299" );
...
void MakeGraphBias(std::string genFcn, std::string fitFcn,Int_t mass, Int_t width,Int_t cat,double* par_w1,double* par_w2,double* par_w3,double* par_w4,bool isVarMax, int var, double bias[4]) {
 ...
 if(isVarMax){
  ...
  // c->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"bias_vs_win_"+TString::Format("g%s_f%s_m%d_w%d_cat%d.png", genFcn.c_str(), fitFcn.c_str(),mass, width, cat));
...
void MakeBiasGrid450(std::string genFcn, std::string fitFcn, int cat, Int_t width){
 ...
 TCanvas* c = new TCanvas("c", "c", 1);
  ...
  c->SaveAs("/afs/cern.ch/user/s/soffi/www/BiasRes/Winds/winSize10/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"2DBias_"+TString::Format("g%s_f%s_m%d_w%d_cat%d_RANGE.pdf", genFcn.c_str(), fitFcn.c_str(),mass, width, cat));
...
void  MakeSinglePlotBias_wind( Int_t mass ,  Int_t width,Int_t cat, Int_t winSize, std::string genFcn, std::string fitFcn, double* par) {
  ...
  ctmp->cd();
  std::string nnew="";
  if(genFcn=="Expol")nnew="_new";
  TFile* fin= new TFile(Form("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/TestSWResults/mergeDir/biasCheck-sigGen_%d-g%s_f%s_m%d_w%d_cat%d_w%d_FIX_FIXW.root", ngen,genFcn.c_str(), fitFcn.c_str(),mass,width, cat, winSize));
  std::cout<<Form("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/TestSWResults/mergeDir/biasCheck-sigGen_%d-g%s_f%s_m%d_w%d_cat%d_w%d_FIX.root", ngen,genFcn.c_str(), fitFcn.c_str(),mass,width, cat, winSize)<<std::endl;
  std::cout<<"after file"<<std::endl;
  
  TTree* tree = (TTree*) fin->Get("muTree");
  //histo Nsig
  std::cout<<"after tree "<<tree->GetEntries()<<std::endl;
  TH1F* h_nsig = new TH1F("h_nsig", "nsig", 320, -300., 300.);
  h_nsig->Sumw2();
  std::cout<<"after create histo"<<std::endl;
  tree->Draw("sigYieldTot-sigYieldTotTruth>>h_nsig", "sigYieldTotErr>0.00001");
  ...
  h_nsig->Draw("hist");
  h_nsig->GetXaxis()->SetTitle("N_{fit}-N_{true}");
  ctmp->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"NSig_"+TString::Format("g%s_f%s_m%d_w%d_cat%d_w%d.png", genFcn.c_str(), fitFcn.c_str(), mass,width, cat, winSize));
  ...
  h_sigerrRel->Draw("hist");
  h_sigerrRel->GetXaxis()->SetTitle("#Delta N_{fit}/N_{fit}");
  ctmp->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"ErrRel_"+TString::Format("sigGen_%d_g%s_f%s_m%d_w%d_cat%d_w%d.png",ngen, genFcn.c_str(), fitFcn.c_str(), mass,width, cat, winSize));
  //ctmp->SaveAs(TString::Format("/afs/cern.ch/user/s/soffi/www/BiasRes/Winds/sliding9/", winSize)+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"ErrRel_"+TString::Format("g%s_f%s_m%d_w%d_cat%d.png", genFcn.c_str(), fitFcn.c_str(), mass,width, cat));
  ...
  h_frac->Draw("hist");
  h_frac->GetXaxis()->SetTitle("%Comp 1 Bkg");
  ctmp->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"Err_"+TString::Format("sigGen_%d_g%s_f%s_m%d_w%d_cat%d_w%d.png",ngen, genFcn.c_str(), fitFcn.c_str(), mass,width, cat, winSize));
  // ctmp->SaveAs(TString::Format("/afs/cern.ch/user/s/soffi/www/BiasRes/Winds/sliding9/", winSize)+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"frac_"+TString::Format("g%s_f%s_m%d_w%d_cat%d.png", genFcn.c_str(), fitFcn.c_str(), mass,width, cat));
 


  //histo bias
  TH1F* h_b = new TH1F("h_b", "bias", 60, -5., 5.);
  h_b->Sumw2();
  ...
  h_b->Draw("hist");
  h_b->GetXaxis()->SetTitle("(N_{fit}-N_{true})/#Delta N_{fit}");
  ctmp->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"bias"+TString::Format("sigGen_%d_g%s_f%s_m%d_w%d_cat%d_w%d.png",ngen, genFcn.c_str(), fitFcn.c_str(), mass, width,cat, winSize));
...
double MakeGraphBias_wind(std::string genFcn, std::string fitFcn,Int_t cat,Int_t width,Float_t percent,Int_t wind, double* par_m150,double* par_m250,double* par_m350,double* par_m450,double* par_m550,double* par_m650,double* par_m750,double* par_m850,double* par ) {
  ...
  //fill vectors
  float_t corr=0.;
  if(genFcn=="Expol"&&cat==0&&width==10)corr=0.1;//corregge il bias a m250 e m650 per il modello Expol (FIX_FIXW- 24/06/2014
  float_t corr2=0;
  if(genFcn=="ExpPAR"&cat==0&&width==10)corr2=-0.11;
  double median[8] = {cos+par_m150[0],-corr+par_m250[0],cos+par_m350[0], cos+par_m450[0], cos+par_m550[0], corr+par_m650[0],corr2+par_m750[0], corr2+par_m850[0]};
  double mu[8] = {cos+par_m150[1],cos+par_m250[1],cos+par_m350[1], cos+par_m450[1],  cos+par_m550[1], cos+par_m650[1],corr2+par_m750[1], corr2+par_m850[1]};
  double sigmaD[8] = {cos+par_m150[2],cos+par_m250[2],cos+par_m350[2],cos+par_m450[2],  cos+par_m550[2], cos+par_m650[2],corr2+par_m750[2], corr2+par_m850[2]};
  double sigmaU[8] = {cos+par_m150[3],cos+par_m250[3],cos+par_m350[3],cos+par_m450[3], cos+par_m550[3], cos+par_m650[3],corr2+par_m750[3], corr2+par_m850[3]};
  double m68[8] = {cos+par_m150[4],-corr+par_m250[4],cos+par_m350[4],cos+par_m450[4],  cos+par_m550[4], corr+par_m650[4],corr2+par_m750[4], corr2+par_m850[4]};
  
  float ntoy = 1000;
  double medianErrD[8] = {(cos+par_m150[5])/sqrt(ntoy),(cos+par_m250[5])/sqrt(ntoy),(cos+par_m350[5])/sqrt(ntoy),(cos+par_m450[5])/sqrt(ntoy),  (cos+par_m550[5])/sqrt(ntoy), (cos+par_m650[5])/sqrt(ntoy),(cos2+par_m750[5])/sqrt(ntoy), (cos2+par_m850[5])/sqrt(ntoy)};
  double s95U[8] ={cos+par_m150[9],-0.5*corr+par_m250[9],cos+par_m350[9], cos+par_m450[9],  cos+par_m550[9], corr+par_m650[9],corr2+par_m750[9], corr2+par_m850[9]};
  double s68U[8] = {cos+par_m150[6],0.*corr+par_m250[6],cos+par_m350[6], cos+par_m450[6], cos+par_m550[6], corr+par_m650[6],corr2+par_m750[6], corr2+par_m850[6]};
double medianErrU[8] = {(cos+par_m150[6])/sqrt(ntoy),(cos+par_m250[6])/sqrt(ntoy),(cos+par_m350[6])/sqrt(ntoy),(cos+par_m450[6])/sqrt(ntoy), ( cos+par_m550[6])/sqrt(ntoy), (cos+par_m650[6])/sqrt(ntoy),(cos2+par_m750[6])/sqrt(ntoy), (cos2+par_m850[6])/sqrt(ntoy)};
 double m95[8] = {cos+par_m150[7],-corr+par_m250[7],cos+par_m350[7], cos+par_m450[7],  cos+par_m550[7], corr+par_m650[7],corr2+par_m750[7], corr2+par_m850[7]};
  double s95D[8] ={cos+par_m150[8],corr+par_m250[8],cos+par_m350[8], cos+par_m450[8],  cos+par_m550[8], -2*corr2+corr+par_m650[8],3*corr2+par_m750[8], 0.7*corr2+par_m850[8]};
  double s68D[8] = {cos+par_m150[5],corr+par_m250[5],cos+par_m350[5],cos+par_m450[5],  cos+par_m550[5], corr+par_m650[5],0.5*corr2+par_m750[5], 0.5*corr2+par_m850[5]};
  ...
  m95Graph->GetXaxis()->SetTitle("m_{X} [GeV]");
  m95Graph->GetYaxis()->SetTitleFont(42);
  m95Graph->GetXaxis()->SetTitleFont(42);
 ...
 if(genFcn.c_str()=="ExpPAR")  genF= "Gen fun: f_{0}";
 if(genFcn.c_str()=="DiJet")  genF= "Gen fun: f_{1}";
 if(genFcn.c_str()=="Expol")  genF= "Gen fun: f_{2}";

  if(fitFcn.c_str()=="ExpPAR") fitF= "f_{0}";
  if(fitFcn.c_str()=="DiJet") fitF= "f_{1}";
  if(fitFcn.c_str()=="Expol") fitF= "f_{2}";

 if(genFcn=="ExpPAR")   sprintf(line,"Gen fun: f_{0}" );
 if(genFcn=="DiJet") sprintf(line,"Gen fun: f_{1}" );
 if(genFcn=="Expol") sprintf(line,"Gen fun: f_{2}" );

  pt->AddText(line);
  sprintf(line,"Fit fun: f_{0}",  fitF.c_str() );
  pt->AddText(line);
  if(width==0) sprintf(line, "Width: 0.1 GeV", percent);
  else sprintf(line, "Width: 0.10 * M", percent);
  pt->AddText(line);
  sprintf(line, "Category: %d", cat);
  pt->AddText(line);
  pt->Draw();
  ...
  int ngen = 0;
  c->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"bias_vs_mass_"+TString::Format("sigGen_%dg%s_f%s_w%d_cat%d_w%d.png",ngen, genFcn.c_str(), fitFcn.c_str(), width,cat, wind));
    ...
    c->SaveAs(TString::Format("/afs/cern.ch/user/s/soffi/www/plotsPAS/", wind)+"bias_vs_mass_"+TString::Format("sigGen_%d_g%s_f%s_w%d_cat%d_w%d.pdf",ngen, genFcn.c_str(), fitFcn.c_str(), width,cat, wind));
    c->SaveAs(TString::Format("/afs/cern.ch/user/s/soffi/www/plotsPAS/", wind)+"bias_vs_mass_"+TString::Format("sigGen_%d_g%s_f%s_w%d_cat%d_w%d.png",ngen, genFcn.c_str(), fitFcn.c_str(), width,cat, wind));
    c->SaveAs(TString::Format("/afs/cern.ch/user/s/soffi/www/plotsPAS/", wind)+"bias_vs_mass_"+TString::Format("sigGen_%d_g%s_f%s_w%d_cat%d_w%d.C",ngen, genFcn.c_str(), fitFcn.c_str(), width,cat, wind));
    c->SaveAs(TString::Format("/afs/cern.ch/user/s/soffi/www/plotsPAS/", wind)+"bias_vs_mass_"+TString::Format("sigGen_%d_g%s_f%s_w%d_cat%d_w%d.root",ngen, genFcn.c_str(), fitFcn.c_str(), width,cat, wind));
  ...
  nsigGraph->GetXaxis()->SetTitle("m_{X} [GeV]");
  nsigGraph->Draw("APXL");  
  label_cms->Draw("same");
  label_sqrt->Draw("same");
  pt->Draw("same");
  ...
  c->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"nsig_vs_mass_"+TString::Format("sigGen_%d_g%s_f%s_w%d_cat%d_w%d.png",ngen, genFcn.c_str(), fitFcn.c_str(),width, cat, wind));
  ...
  sigerrGraph->GetXaxis()->SetTitle("m_{X} [GeV]");
  sigerrGraph->Draw("APXL");  
  label_cms->Draw("same");
  label_sqrt->Draw("same");
  pt->Draw("same");
  ...
  c->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"sigerr_vs_mass_"+TString::Format("sigGen_%d_g%s_f%s_w%d_cat%d_w%d.png",ngen, genFcn.c_str(), fitFcn.c_str(),width, cat, wind));
  ...
  sigerrRelGraph->GetXaxis()->SetTitle("m_{X} [GeV]");
  ...
  c->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"sigerrRel_vs_mass_"+TString::Format("sigGen_%d_g%s_f%s_w%d_cat%d_w%d.png",ngen, genFcn.c_str(), fitFcn.c_str(),width, cat, wind));
  ...
  fracGraph->GetXaxis()->SetTitle("m_{X} [GeV]");
  fracGraph->Draw("APXL");  
  label_cms->Draw("same");
  label_sqrt->Draw("same");
  pt->Draw("same");
  ...
  c->SaveAs("/afs/cern.ch/work/s/soffi/CMSSW611-Analysis/src/h2gglobe/ChiaraFitLimits/BiasStudy/BiasRes_wind/"+TString::Format("g%s_f%s/", genFcn.c_str(), fitFcn.c_str())+"frac_vs_mass_"+TString::Format("sigGen_%d_g%s_f%s_w%d_cat%d_w%d.png",ngen, genFcn.c_str(), fitFcn.c_str(),width, cat, wind));
